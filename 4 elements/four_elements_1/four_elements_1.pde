...
// ELEMENTS
boolean showEarth, showWater, showFire, showAir;
...
void setup() {
  ...
  showEarth = showWater = showFire = showAir = false;
}
...
void draw() {
  ...
  for (int i=0; i < particlePos.length; i++) {
    ...
    //putParticleBackInSandBox(i);

    particleAccel[i].set(0, 0);
  }

  if (showEarth)drawCubes();
  if (showWater)drawSquigglyLines();
  if (showFire)drawFire();
  if (showAir)drawPoints();
...
void drawCubes() {
  ...
  //fill(255,50,0);
  noStroke();
  for (int i=0; i < particlePos.length - 1; i++) {
    float ZtoC = norm(particlePos[i].z, 0, zMultiplier);
    fill(200 * ZtoC, 127 * ZtoC, 0);

    ellipse(particlePos[i].x, particlePos[i].y, 5, 5);

    // FIRST VERTEX
    float gridX = floor((particlePos[i].x / width) * gridResolution);
    float newX = (gridX / gridResolution) * width;
    float gridY = floor((particlePos[i].y / height) * gridResolution);
    float newY = (gridY / gridResolution) * height;

    // SECOND VERTEX
    //gridX = floor((particlePos[i+1].x / width) * gridResolution);
    //gridY = floor((particlePos[i+1].y / height) * gridResolution);
    //float new2X = (gridX / gridResolution) * width;
    //float new2Y = (gridY / gridResolution) * height;

    pushMatrix();
    translate(newX, newY, particlePos[i].z);
    box(gridResolution * 2);
    popMatrix();
    //line(newX, newY, particlePos[i].z, new2X, newY, particlePos[i].z);
    //line(new2X, newY, particlePos[i].z, new2X, new2Y, particlePos[i].z);
  }
  //endShape();
}

void drawSquigglyLines() {
  beginShape();
  noFill();
  stroke(255);
  for (int i=0; i < particlePos.length; i++) {
    stroke(map(particlePos[i].z, 0, zMultiplier, 0, 255), 0, 0);
    curveVertex(particlePos[i].x, particlePos[i].y, particlePos[i].z);
  }
  endShape();
}
...
void drawFire() {
  noStroke();
  for (int i=0; i < particlePos.length; i++) {
    float ZtoColor = norm(particlePos[i].z, 0, zMultiplier);

    fill(ZtoColor * 200,ZtoColor * 127, ZtoColor * 50);
    ellipse(particlePos[i].x, particlePos[i].y, particlePos[i].z, particlePos[i].z);
  }
}
...
void drawPoints() {
  ...
  stroke(255);
  for (int i=0; i < particlePos.length; i++) {
    stroke(map(particlePos[i].z, 0, zMultiplier, 0, 255));
    vertex(particlePos[i].x, particlePos[i].y, particlePos[i].z);
  }
  endShape();
}


void putParticleBackInSandBox(int p) {
  if (particlePos[p].x < 0 || particlePos[p].x > width || particlePos[p].y < 0 || particlePos[p].y > height) {
    particlePos[p].set(random(width), random(height));
  }
}
